...
psnr_,ssim_ = np.zeros(num_classes),np.zeros(num_classes)
for row in range(4):#(num_images):
    # 创建共享的归一化器
    ...
    for col in range(num_classes):
        ...
        if col==0:
        	...
        	psnr_[col]  = psnr_[col] + psnr_val
        	ssim_[col]  = ssim_[col] + ssim_val
...
print(f'类别{class_dirs}')
print(f'平均psnr:{psnr_/num_images}')
print(f'平均ssim:{ssim_/num_images}')
plt.show()
