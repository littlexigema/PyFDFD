...
class Shape:
    ...
    def _validate_lprim_list(lprim_list):
        if not all(isinstance(lprim, torch.Tensor) for lprim in lprim_list):
    ...
    @property
    def cb_center(self):
        return torch.mean(self.bound, axis=1)

    @property
    def L(self):
        l = torch.empty(Axis.count())
        for w in range(Axis.count()):
    ...
    def smoothing_params(self, box):
        ...
        l_probe = [torch.linspace(b[0] - dl / 2, b[1] + dl / 2, self.n_subpxls + 2) for b, dl in zip(box, dl_sub)]

        X, Y, Z = torch.meshgrid(*l_probe, indexing='ij')
        F = self.lsf(X, Y, Z)
        ...
        Fx, Fy, Fz = torch.gradient(F, *dl_sub)
        gradF = torch.stack([Fx[1:-1, 1:-1, 1:-1].flatten(),
                          Fy[1:-1, 1:-1, 1:-1].flatten(),
                          Fz[1:-1, 1:-1, 1:-1].flatten()], axis=1)

        normF = torch.linalg.norm(gradF, axis=1)
        gradF /= normF[:, None]

        ndir = -gradF.sum(axis=0) / torch.linalg.norm(gradF.sum(axis=0))
        return rvol, ndir